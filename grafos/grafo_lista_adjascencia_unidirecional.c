...
#include <stdlib.h>
#include <assert.h>
#include "grafo_lista_adjascencia_unidirecional.h"
...
int main(void) {
	...
	adiciona_ou_modifica_grafo(grafo, 1, NULL, 0);
	adiciona_ou_modifica_grafo(grafo, 2,(struct Vertice *[]) {procura_vertice(grafo,1)}, 1);
	adiciona_ou_modifica_grafo(grafo, 3,(struct Vertice *[]) {procura_vertice(grafo,1)}, 2);
	...
	assert(deletar_vertice(grafo, 4) == 0);
